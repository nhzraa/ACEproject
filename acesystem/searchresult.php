...
<title>Search Result (Staff)</title>
...
<div class="content">
<h3 align="center"><span style="padding-left:250px">STAFF DETAILS: Search Result</span> 
<span style="float:right"><span style="padding-right:150px"><a href="staff-register.php" class="btn info">+ NEW STAFF</a></span></span></h3>

<br><br>
<form action="searchresult.php" method="get" style="float:right; padding-right:150px">
  <span>Search Staff Status: </span>
  <input name="search" type="text" placeholder="Full Time/Part Time/Internship"> 
  <input name="btnsearch" type="submit" value="GO"></form>
<br>
<br>

<table width="87%" align="center"  class="table" >
      <tr>
    ...
    $sql = "select * from `admin` where `adminStatus` like '%$search%' order by adminStatus asc";
    ...
    while($row = mysqli_fetch_assoc($result))
	{
        ...
        Print '<tr>
		    	  <td><center>'.$row['adminID'].'</center></td>
                  <td>'.$row['adminName'].'</td>
                  <td><center>'.$row['adminPhone'].'</center></td>
                  <td><center>'.$row['adminEmail'].'</center></td>
				  <td>'.$row['adminAddress'].'</td>
				  <td><center>'.$row['adminPosition'].'</center></td>
				  <td><center>'.$row['adminStatus'].'</center></td>
                  <td>
                   <center>
				  <a href="staff-update.php?adminID='.$row['adminID'].'" class="btn update" >Update</a>
				  
				  <a href="deleteprocess.php?adminID='.$row['adminID'].'" class="btn delete" >Delete</a>
